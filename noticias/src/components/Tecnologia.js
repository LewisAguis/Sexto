...
export default class Tecnologia extends Component {
    ...
    Escuchar(){
        var doc = firebase.collection('Noticias').where('Categoria','==','Tecnologia').orderBy("Likes",'desc');
	...
	getNoticias(){
        ...
        firebase.collection('Noticias').where('Categoria','==','Tecnologia').orderBy("Likes",'desc').get().then(snapshot => {
            var tmp=[];
