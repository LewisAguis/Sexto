...
export default class Deportes extends Component {
	...
	Escuchar(){
        var doc = firebase.collection('Noticias').where('Categoria','==','Deportes').orderBy("Likes",'desc');
	...
	getNoticias(){
        ...
        firebase.collection('Noticias').where('Categoria','==','Deportes').orderBy("Likes",'desc').get().then(snapshot => {
            var tmp=[];
